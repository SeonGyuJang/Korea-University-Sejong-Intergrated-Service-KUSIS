...
def crawl_and_save_menu():
    """고려대학교 세종캠퍼스 식당 메뉴를 크롤링하여 JSON 파일로 저장"""
    ...
    try:
        ...
        diet_menus = soup.find_all('div', class_='diet-menu')
        if len(diet_menus) < 2:
            print("식단표를 찾을 수 없습니다.")
            return None

        student_menu = diet_menus[1]
        staff_menu = diet_menus[0]
